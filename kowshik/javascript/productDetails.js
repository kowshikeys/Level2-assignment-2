const products = [
    {
      ...
      image:
        "https://www.tanisq..in/wps/wcm/connect/tanishq/b629da6f-83f0-465d-ad64-7dfa6e871abc/images%252Fsecondary-collection-banner-mobile.jpg?MOD=AJPERES&CVID=",
    },
    {
      id: 2,
  ...
  window.onload = ()=>{
    ...
    const productOne = products.find((pro)=>pro.id === Number(name))
  ...
  cart.innerHTML = productOne.cart
  ...
  cart.addEventListener("click", function (e) {
    ...
    if (!cartOld) {
      localStorage.setItem("cart", JSON.stringify([productOne.id]));
      return;
    }

    const isOldId = cartOld.find((p) => p === productOne.id);

    if (isOldId) {
      const filterData = cartOld.filter((p) => p !== productOne.id);
      localStorage.setItem("cart", JSON.stringify(filterData));
      cart.innerHTML = "cart";
      return;
    }

    cartOld.push(productOne.id);
