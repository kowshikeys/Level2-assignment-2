...
const sortProducts = document.getElementById("priceFilter")
const carts = (parentElement, i) => {
  ...
  a.onclick = () => {
    window.location.href = `http://127.0.0.1:5500/kowshik/Public/Html/productDetails.html?id=${i.id}`;
  };
...
const search = (value) => {
  ...
  const filterValue = (products, term) => {
    let f = products.filter(function (a) {
  ...
  const filtering = filterValue(products, value)
...
const startingPrice = document.getElementById("start-price");
const endingPrice = document.getElementById("end-price");
const filterButton = document.getElementById("button-filter")
const closes = document.querySelector(".filter-icon")

filterButton.addEventListener("click",() =>{
   let productFilter = products.filter(function (sort){
    closes.style.display="flex"
    return sort.price>=startingPrice.value && sort.price<=endingPrice.value
    
   })
   
   console.log(productFilter)
   allProducts.remove()
   for(let i of productFilter){
    carts(sortProducts,i)
   }
}) 
