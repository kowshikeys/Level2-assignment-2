...
window.onload = () => {
  ...
  like.innerHTML = singleProduct.like;
  ...
  like.addEventListener("click", function (e) {
    ...
    if (!cartOld) {
      localStorage.setItem("like", JSON.stringify([singleProduct.id]));
      return;
    }

    const isOldId = cartOld.find((p) => p === singleProduct.id);

    if (isOldId) {
      const filterData = cartOld.filter((p) => p !== singleProduct.id);
    ...
    localStorage.setItem("like", JSON.stringify(cartOld));
